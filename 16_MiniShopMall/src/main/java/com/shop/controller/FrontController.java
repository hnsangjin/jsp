...
public class FrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// "/프로젝트명/파일명(*.go)" 라는 문자열을 반환해주는 메서드.
		...
		/*
		 * Properties 클래스 - java.util 패키지에 존재하는 클래스. - properties 클래스는 HashTable의 하위(자식)
		 * 클래스임. - 보통은 환경변수 및 속성 값을 Properties 파일에 저장하여 쉽게 접근할 수 있는 장점이 있음 - Properties
		 * 파일은 일련의 (key) - 값(value)의 한 쌍으로 이루어져 있음. - 보통은 파일에 저장을 하여 사용을 함. 파일 이름은
		 * *.properties 라는 이름으로 끝나게 함. - FileInputStream 클래스에 properties 파일을 인자로 넣어서 그
		 * 스트림으로부터 파일을 가져올 때 많이 사용함. 인자로 들어온 properties 파일을 읽게 됨. - 읽어들일 때 사용하는 메서드는
		 * load() 라는 메서드를 이용하여 파일을 읽어 들이게 됨.
		 * 
		 */

		FileInputStream fis = new FileInputStream(
				"C:\\Users\\goott3\\git\\jsp\\16_MiniShopMall\\src\\main\\java\\com\\shop\\controller\\mapping.properties");
		...
		if (value.substring(0, 7).equals("execute")) {
			...
			try {
				...
				System.out.println("Class.forName(url_2) >>> " + Class.forName(url_2));
				// url.newInstance();
				...
				// 동적으로 로딩된 클래스(객체)의 생성자(기본생성자)
				Constructor<?> constructor = url.getConstructor();
				System.out.println("constructor" + constructor);
				// 가져온 기본생성자를 이용하여 newInstance() 메서드를 호출하여 객체를 생성하는 메서드.
		...
		if (forward != null) {
			...
			if (forward.isRedirect()) {// true 인 경우
				...
				System.out.println("forward.getPath()1 >>> " + forward.getPath());
			}
		}
	}
